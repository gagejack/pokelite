...
**Architecture:** A pure-data module maps all 72 move names to sound-file stems (no asset imports, so Node can test it). A thin binding module imports the 53 `.m4a` files and joins them to that table. `BattleCard` plays the resolved URL at projectile launch. Source WAVs are converted offline to 48 kbps mono AAC — 18.7 MB becomes under 1 MB.
...
- **Node cannot import asset files.** Importing a module that statically imports `.png`/`.m4a` fails with `ERR_UNKNOWN_FILE_EXTENSION`. Any module a test imports must contain only plain JS. This is why the mapping is split into pure-data modules (`moveSounds.data.js`, `moveAliases.data.js`) separate from the asset-importing `moveSounds.js`.
...
| `src/game/moveAliases.data.js` (create) | `MOVE_ANIMATION_ALIASES`, moved out of `moveAnimations.js` so tests can import it. |
| `src/game/moveAnimations.js` (modify) | Re-export the alias table from its new home. |
...
This module holds **no asset imports** so `node --test` can import it. The 18 moves that the spec resolves through `MOVE_ANIMATION_ALIASES` are inlined here as already-resolved stems, because `moveAnimations.js` statically imports 79 PNGs and would make this module untestable. Task 2b adds the test that keeps those inlined copies honest.
...
### Task 2b: Guard the inlined alias entries

**Files:**
- Create: `src/game/moveAliases.data.js`
- Modify: `src/game/moveAnimations.js:171-200` (cut the table, re-export it)
- Modify: `src/game/moveSounds.data.test.js` (add one test)

**Interfaces:**
- Consumes: nothing new.
- Produces: `MOVE_ANIMATION_ALIASES: Record<string, string>` from
  `src/game/moveAliases.data.js`. `moveAnimations.js` re-exports it, so every
  existing importer keeps working unchanged.

Task 2 inlined 18 alias-resolved stems into `MOVE_SOUND_FILES` as copies. Nothing
currently stops someone editing `MOVE_ANIMATION_ALIASES` and leaving those copies
stale — and no test can catch it while the alias table lives in a file that
imports 79 PNGs. Moving the table into its own import-free module makes the check
possible.

- [ ] **Step 1: Create the pure alias module**

Create `src/game/moveAliases.data.js`. Cut lines 171-200 of
`src/game/moveAnimations.js` (the whole `export const MOVE_ANIMATION_ALIASES = {
... }` block, including its leading comment at lines 166-170) and paste them in
under this header:
...
- [ ] **Step 2: Re-export from `moveAnimations.js`**

In `src/game/moveAnimations.js`, where the table used to be, put:
...
Then add this import at the top of the file, beside the other imports:

```js
import { MOVE_ANIMATION_ALIASES } from './moveAliases.data.js'
```

The existing `getMoveAnimation` at the bottom of the file still references
`MOVE_ANIMATION_ALIASES` and now resolves it through that import.

- [ ] **Step 3: Write the failing test**

Append to `src/game/moveSounds.data.test.js`:
...
Expected: PASS. The drift list is empty — every alias-resolved move's inlined
stem matches its target's. If it fails, the mapping in `moveSounds.data.js` and
the alias table disagree; fix the mapping, not the test.

- [ ] **Step 5: Verify the app still builds**

Run: `npm run build`

Expected: succeeds. This confirms the `moveAnimations.js` re-export did not break
its own `getMoveAnimation`.

- [ ] **Step 6: Commit**

```bash
git add src/game/moveAliases.data.js src/game/moveAnimations.js src/game/moveSounds.data.test.js
git commit -m "refactor(sfx): extract alias table, guard inlined sound stems"
```
...
4. Winning still plays the level-up cue, and it is audibly louder than the attacks. **Desktop only** — `BattleCard.jsx` gates the level-up cue on `isDesktop`, because the `+N LVL` popups only render there. Skip this check at mobile widths.
